...
namespace Tasslehoff.Telnet
{
    ...
    public class TelnetThread
    {
        ...
        private void ConnectionThread()
        {
            ...
            var asyncCallback = new AsyncCallback(this.ReadCallback);
            ...
            while (!this.clientThreadCancelled)
            {
                ...
                if (asyncResult == null || asyncResult.IsCompleted)
                {
                    // try
                    // {
                    asyncResult = this.stream.BeginRead(buffer, 0, buffer.Length, asyncCallback, null);
                    // }
                    // catch
                    // {
                    //     break;
        ...
        private void ReadCallback(IAsyncResult asyncResult)
        {
            ...
            try
            {
                read = this.stream.EndRead(asyncResult);
            }
            catch
            {
                return;
            ...
            this.stringBuffer += this.server.Encoding.GetString(buffer, 0, read);
